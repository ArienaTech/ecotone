...
const categoryColors: Record<string, string> = {
  'Emulsion Stains': '#2d6a4f',
  'Protective Coatings': '#1a6b6b',
  'Decorative Finishes': '#4a4a8a',
  'Mineral Coatings': '#6a4a2d',
  'Primers & Preparation': '#6a6a2d',
  'Protective Sealers': '#4a2d6a',
  'Waterproof Systems': '#2d4a6a',
  'Cleaning Solutions': '#6a2d4a',
  'Functional Coatings': '#1a6b3b',
};
  ...
  return (
    <div style={{ overflowX: 'hidden' }}>
      ...
      <main style={{ paddingTop: '64px' }}>
        ...
        <section style={{ background: '#2d6a4f', padding: '48px 6%' }}>
              ...
              {products.map((product) => (
                ...
                >
                  <span
                    style={{
                      display: 'inline-block',
                      background: categoryColors[product.category] || '#2d6a4f',
                      color: '#ffffff',
                      fontFamily: 'Inter',
                      fontWeight: 500,
                      fontSize: '10px',
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    <p style={{ fontFamily: 'Inter', fontWeight: 500, fontSize: '11px', color: '#2d6a4f', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '8px' }}>
                      Key Features
                    </p>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                      {product.features.map((f) => (
                        <span key={f} style={{ background: '#f0f8f4', border: '1px solid #b8ddc8', color: '#1a4a35', fontFamily: 'Inter', fontWeight: 400, fontSize: '11px', padding: '3px 10px' }}>
                          {f}
                        </span>
                      ))}
                    </div>
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    {product.applications.map((a) => (
                      <p key={a} style={{ fontFamily: 'Inter', fontWeight: 400, fontSize: '12px', color: '#606060', marginBottom: '4px', display: 'flex', gap: '6px' }}>
                        <span style={{ color: '#2d6a4f', flexShrink: 0 }}>→</span>{a}
                      </p>
                    ))}
                  </div>
                  ...
                  <a
                    ...
                    onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.background = '#2d6a4f'; e.currentTarget.style.color = '#ffffff'; }}
                    onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#2d6a4f'; }}
                  >
                    Datasheet ↓
                  </a>
                </div>
